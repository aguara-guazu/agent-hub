...
it.skipIf(!binary)('OpenCode real devuelve JSON con sólo StructuredOutput permitido y se cierra con el worker', async () => {
  ...
  const provider = createServer(async (req, res) => {
    ...
    const chunk = { id: 'chatcmpl_fixture', object: 'chat.completion.chunk', created: 1, model: 'fixture', choices: [{ index: 0,
      delta: { role: 'assistant', tool_calls: [{ index: 0, id: 'call_fixture', type: 'function', function: { name: 'StructuredOutput', arguments: JSON.stringify({ facts: ['Hecho de prueba'] }) } }] }, finish_reason: null }] }
  ...
  try {
    ...
    expect(requests[0].tools.map((t: any) => t.function.name)).toEqual(['StructuredOutput'])
